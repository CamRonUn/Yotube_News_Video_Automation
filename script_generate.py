    ...
    for part in parts:
        ...
        def gen_part(part_, current_accumulated_plan):
            ...
            i need you to return a json file that looks like exactly like this ### {json_style_guide} ###
                ...
                the video title im sending from the tanscript of is found in the part between the || ... || use it the large majority of scenes should have video and time frames just guese the time stamps in video that would be good based on transcript to show. and try make time of clip the same as a estimation of the voice over. include the full video title not just the code
                ...
                MAKE SURE SCRIPT IS NOT REPETETIVE DO NOT REPEAT ANYTHING FROM THE full script if you must make the part less scenes to avoic repetition
                if your going to set video: x due to no relavent video pick a random video from the following list ### {titles} ### and use start and end duration a random time withing first 2 mins of the videos if you have relavent part of the video to show based on subtitles show that part.

                this is the last 100 characters of the script before your part, continue from this do not repeat this, if nothing inbetween ### ### this is the intro else this is not the intro do not make an intro ### {current_accumulated_plan[:100] if len(current_accumulated_plan) else ""} ### try make the whole script flow well and make sense. do not repeat yourself and focus on the part of the full script ive given you. 
